...
const initialPosts = [
  { id: 1, category: "질문", author: "자취 2년차", time: "12분 전", title: "자취하면서 요리 시작했는데 간단한 레시피 추천해주세요!", body: "퇴근 후 15분 안에 만들 수 있는 메뉴면 더 좋아요.", likes: 24, comments: 17, color: "mint" },
  { id: 2, category: "고민상담", author: "취준하는 청년", time: "34분 전", title: "면접 준비 같이 하실 분 구해요!", body: "서로 예상 질문 봐주고 피드백하면 좋을 것 같아요.", likes: 15, comments: 8, color: "purple" },
  { id: 3, category: "자유", author: "독서 좋아하는 사람", time: "5시간 전", title: "이번 주말 독서 모임 모집합니다", body: "가볍게 읽은 책 이야기 나누는 모임이에요.", likes: 18, comments: 12, color: "yellow" },
  { id: 4, category: "정보공유", author: "강서구 주민", time: "어제", title: "강서구 근처 조용한 카페 추천해요", body: "작업하기 좋고 콘센트 많은 곳 위주로 정리했어요.", likes: 31, comments: 14, color: "blue" },
];
...
const supportItems = [
  { id: 1, title: "청년 월세 한시 지원", status: "신청 가능", desc: "월 최대 20만원, 12개월 지원", category: "주거", target: "만 19-39세 무주택 청년", period: "2026.06.01 - 2026.07.15", icon: Home, color: "mint" },
  { id: 2, title: "구직활동 응원 패키지", status: "맞춤 추천", desc: "면접 정장, 사진, 컨설팅 바우처", category: "취업", target: "구직활동 중인 청년", period: "상시 접수", icon: ClipboardCheck, color: "blue" },
  { id: 3, title: "마음건강 상담권", status: "오늘 마감", desc: "전문 상담 1:1 연계", category: "마음", target: "심리 상담이 필요한 청년", period: "2026.06.21 마감", icon: HeartHandshake, color: "coral" },
  { id: 4, title: "청년 교육 클래스", status: "추천", desc: "디자인·영상·취업 역량 강화", category: "교육", target: "역량 강화를 원하는 청년", period: "2026.06.25 - 2026.08.30", icon: Sprout, color: "purple" },
];
    ...
    <ScreenFrame>
      <div className="flex min-h-[100dvh] flex-col justify-between bg-white px-6 py-7 sm:min-h-[860px]">
        ...
        <section className="space-y-6">
          <div className="relative mx-auto h-[280px] overflow-hidden rounded-[30px] border border-slate-100 bg-white shadow-card">
            <PastelBlob className="left-7 top-8 h-24 w-24 bg-mint/20" />
            <PastelBlob className="right-8 top-16 h-20 w-20 bg-purple/20" />
            <PastelBlob className="bottom-14 left-9 h-20 w-20 bg-blue/20" />
            <PastelBlob className="bottom-16 right-10 h-20 w-20 bg-yellow/30" />
            <div className="absolute left-1/2 top-9 flex h-28 w-28 -translate-x-1/2 items-center justify-center rounded-[30px] bg-white p-5 shadow-lift">
            ...
            <div className="absolute bottom-24 left-0 right-0 flex items-end justify-center gap-5">
              ...
              <div className="flex h-24 w-14 flex-col items-center justify-center rounded-full bg-white shadow-card">
                <Sparkles className="text-purple" size={23} />
                <span className="mt-2 h-8 w-2 rounded-full bg-purple/30" />
              </div>
              <AvatarBubble icon={HeartHandshake} color="blue" />
            </div>
            <div className="absolute bottom-6 left-6 right-6 rounded-3xl bg-white/90 px-5 py-3 text-center shadow-lift backdrop-blur">
...
function MobileShell(props) {
  ...
  return (
    <ScreenFrame>
      <div className="flex min-h-[100dvh] flex-col bg-white sm:min-h-[860px]">
...
function AppModal(props) {
  const { modal, setModal, setActiveTab, setMoodResult, setPosts, joinedMeetups, setJoinedMeetups, savedSupports, setSavedSupports, closeModal } = props;
  ...
  if (modal.type === "actions") {
    const actions = [
      ...
      { label: "심리테스트 하기", icon: BookOpenCheck, color: "coral", run: () => { setMoodResult("차분한 회복형"); setActiveTab("home"); closeModal(); } },
    ];
    return (
...
function CompactMapCard() {
  return (
    <section className="rounded-[24px] bg-white p-4 shadow-card">
      ...
      <div className="relative mt-3 h-20 rounded-2xl bg-[#F8FAFC]">
        <div className="absolute inset-0 opacity-60 map-grid" />
        <div className="absolute left-1/2 top-1/2 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-mint shadow-lift" />
        <span className="absolute left-2 top-2 rounded-full bg-mint/10 px-2 py-1 text-[10px] font-black text-mint">재학생</span>
        <span className="absolute bottom-2 right-2 rounded-full bg-purple/10 px-2 py-1 text-[10px] font-black text-purple">자취중</span>
      </div>
    </section>
  );
}

function CompactWalkCard() {
  return (
    <section className="rounded-[24px] bg-white p-4 shadow-card">
      <div className="flex items-center justify-between">
      ...
      <h2 className="mt-3 text-2xl font-black text-ink">5,300</h2>
      <p className="text-xs font-bold text-sub">목표 10,000보</p>
      <ProgressBar value={53} color="mint" compact />
      <p className="mt-2 text-xs font-black text-mint">완료 시 50P</p>
    </section>
  );
}
...
function ScreenFrame({ children }) {
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-ink">
